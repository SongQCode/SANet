    ...
    def cal_cosinesimilarity(self, input1, input2):
        ...
        for i in range(B):
            ...
            for j in range(len(input2)):
                ...
                wdata = torch.cat(([shotList[i] for i in range(shot_num)]), 0)
                wdata2 = torch.pow((1 - torch.var(wdata, dim=0, unbiased=False)),H)
                dataSum = torch.sum(wdata, dim=0)
                intraWeight = dataSum + t*wdata2
                interWeight = torch.var(matrix, dim=0, unbiased=False)
                fuseWeight=intraWeight*interWeight
                topk_value, topk_index = torch.topk(listM[j], self.neighbor_k, 1)
                sim = torch.sum(topk_value, dim=1)
                data = sim * fuseWeight
                inner_sim[0, j] = torch.sum(data)
